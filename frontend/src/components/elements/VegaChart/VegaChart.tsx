...
/**
 * Fix bug where Vega was vertically-cropping the x-axis in some cases.
...
class VegaChart extends React.PureComponent<PropsWithHeight, State> {
  /**
  ...
  public render(): JSX.Element {
    ...
    return (
      ...
      <div className="stVegaChart" ref={c => (this.element = c)} />
    )
  }
  ...
  private async createView(): Promise<void> {
    ...
    // Initialize the data with dataset names so the spec can find them
    var data_spec = []
    for (const name of datasetNames) {
      data_spec.push({ name: name, values: [] })
    }
    spec["data"] = data_spec
...
function getDataSets(
  ...
  if (!el.get("data") || el.get("data").isEmpty()) {
    return null
  }

  const datasets: { [dataset: string]: any } = {}

  el.get("data").forEach((x: any, i: number) => {
    if (!x) {
      return
    }
...
export default withFullScreenWrapper(VegaChart)
